 ...
 * using the given compression type and thread counts. Then re-open the file 
 * with an mtbl_reader and verify that we get the exact same key/value entry back.
 *
 * This tests that both threaded data block compression and unthreaded 
 * decompression are functioning correctly.
 *
 ...
 */
static int
test_threaded_compression(mtbl_compression_type c_type, 
			  size_t thread_count, 
			  const char *dirname)
{
	mtbl_res res;
